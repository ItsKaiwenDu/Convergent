...
-   **Multi-Format Support**:
    ...
    -   **OCR**: Extract text from images (`JPG`/`JPEG`/`PNG`/`HEIC`) and save results locally to plain text (`.txt`), Markdown (`.md`), Word Document (`.docx`), or PDF format (uses macOS native Vision API or Tesseract fallback; HEIC is auto-converted to PNG internally via `sips`).
